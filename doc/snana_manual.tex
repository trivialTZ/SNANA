...
# Optional keys:
  ...
  PSF_MODEL:   MOFFAT           # optional PSF model (default: GAUSS)
  MOFFAT_BETA: <VAL>            # wing parameter; required for PSF_MODEL: MOFFAT
...
%
  \item {\bf PSF1,2 and RATIO:} The PSF is specified by a double-Gaussian
	...
	Alternatively, to simulate more realistic PSF wings, a normalized
	Moffat profile can be enabled with global-header keys
	{\tt PSF\_MODEL: MOFFAT} and {\tt MOFFAT\_BETA: <VAL>}.
	For {\tt PSF\_UNIT: ARCSEC\_FWHM}, {\bf PSF1} is interpreted as the
	Moffat FWHM (and {\bf PSF2},{\bf RATIO} should be zero/ignored).
	For {\tt PSF\_UNIT: NEA\_PIXEL} (or {\tt NEA\_ARCSECSQ}), the NEA value
	is interpreted as the Moffat NEA and SNANA computes the corresponding
	FWHM from ({\tt NEA},{\tt MOFFAT\_BETA}).
%
  \item {\bf NEA:} Rather than providing 3 PSF parameters, the noise-equiv-area
       ...
       If {\tt PSF\_MODEL: MOFFAT} is enabled, the NEA values are interpreted
       as Moffat NEA (rather than Gaussian NEA).
%
...
If {\tt PSF\_MODEL: MOFFAT} is enabled, SNANA uses an equal-area
circle approximation for the central pixel and the enclosed-energy
fraction for a normalized Moffat profile:
$$
 \fA \approx 1 - \left[1 + \left(\frac{r_{\rm eff}}{\alpha}\right)^2 \right]^{1-\beta},
 \quad r_{\rm eff} \equiv \frac{P}{\sqrt{\pi}},
$$
where $\beta$ is {\tt MOFFAT\_BETA} and $\alpha$ is the Moffat scale radius.
